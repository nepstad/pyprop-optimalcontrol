...
class Krotov(OptimalControl):
	...
	def SetupVectorB(self, timeGridIndex, direction):
		...
		for a in range(self.NumberOfControls):
			...
			h = self.TimeGridResolution
			self.b[a] -= 1 / h * self.PenaltyMatrix.DotRow(timeGridIndex, self.ControlVectors[a,:])
	...
	def SetupMatrixM(self, timeGridIndex, direction):
		...
		for a in range(self.NumberOfControls):
			for b in range(self.NumberOfControls):
				if a == b:
					self.M[a,b] = self.PenaltyMatrix.GetDiagonalElement(timeGridIndex) / self.TimeGridResolution
